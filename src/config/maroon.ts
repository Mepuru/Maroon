...
/** 出现在 RSS Feed 中的 collection id 列表 */
export function generateRssCollections(): string[] {
  return contentRegistry.filter((c) => c.rssIncluded !== false).map((c) => c.id);
}
